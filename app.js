...
async function fetchWeather() {
  ...
  try {
    ...
    const [weatherResp, cityName] = await Promise.all([
      fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude.toFixed(2)}&longitude=${longitude.toFixed(2)}&current_weather=true`).then(r => r.json()),
...
async function loadWeatherDisplay() {
  ...
  if (!dateEl) return;
  // 先显示加载中
  if (!dateEl.querySelector('.hero-weather-inline')) {
    dateEl.insertAdjacentHTML('beforeend', ' <span class="hero-weather-inline" style="opacity:0.4">⏳</span>');
  }
  const data = await fetchWeather();
  const inline = dateEl.querySelector('.hero-weather-inline');
  if (data && inline) {
    const cityStr = data.city ? ` ${data.city} ` : ' ';
    inline.innerHTML = `${getWeatherEmoji(data.code)} ${data.temp}°C${cityStr}`;
...
function renderHome(main) {
  ...
  main.innerHTML = `
    <div class="home-hero">
      ...
      <div class="hero-date" id="liveDate">${formatDateCN(today)}</div>
